...
//   a=67    // let variables can be reassigned but caanot re-declare within same functional and block scope
...
if(true){
    ...
    var c= 99 // that's the problem with var in every scope(block and functional ) its value will be changed and will be the same 
console.log(c);
...
// primitive data types -> 7 (number,string,bigint,boolean,null,undefined,symbol) #call by value ->originals won't be chnaged only copy
// non-primitive data types -> 3 (object,array,function) #call by reference ->originals will be chnaged

//depending upon how variables can be stored in the memory and retrived we have two memory types:
